...
def _fetch_bdc_payments(oid: ObjectId):
    ...
    for p in direct + legacy:
        ...
        if isinstance(dt, str):
            try: p["date"] = datetime.fromisoformat(dt)
            except Exception: p["date"] = None
        out.append(p)
...
def _compute_current_balance(bdc_id: ObjectId):
    ...
    from_acc_paid = credit_paid = 0.0
    from_acc_pending = credit_pending = 0.0
    for p in _fetch_bdc_payments(oid):
        ptype = (p.get("payment_type") or "").strip().lower()
        amount = _to_f(p.get("amount"))
        status = _norm_status(p)
        if ptype == "from account":
            (from_acc_paid if status == "paid" else from_acc_pending).__iadd__(amount)
        elif ptype == "credit":
            (credit_paid if status == "paid" else credit_pending).__iadd__(amount)
...
def _pending_map_for_all_bdcs():
    ...
    Only counts 'from account' and 'credit' where bank_status != 'paid'.
...
# 📄 View All BDCs (now passes unpaid flags; card no longer shows balance)
@bdc_bp.route('/bdc')
def bdc_list():
...
# ✏️ Edit/Update BDC details (name/phone/location/rep fields)
...
def update_bdc(bdc_id):
    ...
    # remove empty keys (allow partial updates)
...
# 💰 Manual Deposit (unchanged logic)
...
# 👤 BDC Profile (kept, balance computed not stored)
